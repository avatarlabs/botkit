...
function WebBot(configuration) {
    ...
    controller.openSocketServer = function(server) {
        ...
        wss.on("connection", function connection(ws) {
            ...
            ws.on("message", function incoming(message) {
                ...
                controller.activeBots[message.user] = bot;
                bot.activeKey = message.user;
            });
            ...
            ws.on("close", function(err) {
                ...
                // kill the active bot
                if (bot.activeKey) {
                    delete controller.activeBots[bot.activeKey];
                }
            });
        });
    ...
    controller.middleware.ingest.use(function(
        bot,
        message,
        reply_channel,
        next
    ...
    controller.defineBot(function(botkit, config) {
                ...
                if (bot.ws) {
                    try {
                        if (bot.ws && bot.ws.readyState === WebSocket.OPEN) {
                            ...
                            console.error(
                                "Cannot send message to closed socket"
                            );
                        }
                    } catch (err) {
                        return cb(err);
                    ...
                    if (bot.ws && bot.ws.readyState === WebSocket.OPEN) {
                        bot.ws.send(
                            ...
                            function(err) {
                                if (err) {
                                    console.error(
                                        "startTyping failed: " + err.message
                                    );
                                }
                            }
                        );
        ...
        bot.typingDelay = function(message) {
            return new Promise(function(resolve, reject) {
                ...
                if (message.typingDelay) {
                    ...
                    typingLength = Math.min(
                        Math.floor(textLength / (avgCPM / 60)) * 1000,
                        2000
                    );
                }
        ...
        bot.reply = function(src, resp, cb) {
            ...
            if (
                resp.typing ||
                resp.typingDelay ||
                controller.config.replyWithTyping
            ) {
