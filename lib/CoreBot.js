...
// Fisher-Yates shuffle algo https://stackoverflow.com/questions/6274339/how-can-i-shuffle-an-array
...
function Botkit(configuration) {
    ...
    botkit.middleware = {
        ...
        categorize: ware(),
        receive: ware(),
        receiveOutsideOfConversation: ware(),
            ...
            if (err) {
                ...
                bot.findConversation(message, function(convo) {
                    if (convo) {
                        ...
                        botkit.middleware.receiveOutsideOfConversation.run(bot, message, function(
                            err,
                            bot,
                            message
                        ) {
                            if (err) {
                                console.error(
                                    'An error occured in the receiveOutsideOfConversation middleware: ',
                                    err
                                );
                                return;
                            } else {
                            ...
                            if (message.conditional) {
                                ...
                                if (message.text || message.attachments || message.attachment) {
                                    ...
                                    this.task.bot.reply(this.source_message, outbound, function(err, sent_message) {
                                        // now execute any postAction function we've attached regardless of success or not
                                        if (message.postAction) {
                                            message.postAction(that);
                                        }
    ...
    botkit.trigger = function(event, data) {
        if (this.events[event]) {
            ...
            // make a randomized array so that not always first registered conversation
            var random_hearing = hearing.slice();
            // Fisher-Yates shuffle algo https://stackoverflow.com/questions/6274339/how-can-i-shuffle-an-array
            shuffleArray(random_hearing);

            // first, look for hearing type events
            // these are always handled before normal event handlers
            for (var e = 0; e < random_hearing.length; e++) {
                var res = random_hearing[e].callback.apply(this, data);
                if (res === false) {
                    return;
                }
            }
            ...
            // make a randomized array so that not always first registered conversation
            var random_handler = handlers.slice();
            // Fisher-Yates shuffle algo https://stackoverflow.com/questions/6274339/how-can-i-shuffle-an-array
            shuffleArray(random_handler);

            botkit.debug('handlers is juggling ', random_handler.length);

            // now, if we haven't already heard something,
            // fire the remaining event handlers
            if (random_handler.length) {
                botkit.middleware.triggered.run(data[0], data[1], function(err, bot, message) {
                    for (var e = 0; e < random_handler.length; e++) {
                        var res = random_handler[e].callback.apply(this, data);
                        if (res === false) {
                            return;
                        }
                    }
